...
class LSTMNetwork(nn.Module):
    ...
    def forward(self, x: torch.Tensor, future_steps: int = 1) -> torch.Tensor:
        ...
        for _ in range(future_steps):
            ...
            # Reconstruct full input: predicted position + time features
            # Assuming input_dim = 4 (lat, lon, alt, time)
            if self.input_dim == 4:
                # Extract last time and add prediction interval
                last_time = x[:, -1, 3:4]  # [batch_size, 1]
                next_time = last_time + 60  # Add 60 seconds for next minute
                # Concatenate predicted position with time
                current_input = torch.cat([output, next_time.unsqueeze(1)], dim=-1)
            else:
                # For other input dimensions, pad with zeros or use last hidden state
                # This maintains the correct input dimensions
                padding = torch.zeros(
                    batch_size, 1, self.input_dim - self.output_dim
                ).to(x.device)
                current_input = torch.cat([output, padding], dim=-1)

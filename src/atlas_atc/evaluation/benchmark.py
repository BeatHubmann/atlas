...
class TrajectoryBenchmark:
    ...
    def __init__(self, data_path: str, results_path: str | None = None):
    ...
    def train_models(self, data_splits: dict[str, np.ndarray]) -> None:
        ...
        self.models["lstm"].fit(X_train, y_train)
        ...
        start_time = time.time()
        self.models["transformer"].fit(X_train, y_train)
...
def main() -> dict[str, Any]:
    ...
    benchmark = TrajectoryBenchmark(
        data_path=str(settings.DATA_DIR / "scat"),
        results_path=str(settings.RESULTS_DIR / "experiments")
    )
